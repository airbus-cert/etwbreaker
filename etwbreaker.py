...
Actually ETWBreaker can work with Manifest based ETW and Tracelogging.

For Manifest based ETW, ETWBreaker will parse module ressources to find and parse the manifest
and show all possible event handled by the current module.

It try to parse also all tracelogging event, which are more hidden than Manifest.

At the end ETWBreaker can generate a conditional breakpoint to dynamically analyse event.
"""
